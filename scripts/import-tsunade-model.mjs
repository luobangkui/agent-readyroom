// Import the complete user-supplied Tsunade GLB as an office actor.
...
const sourceURL=new URL('../assets/imports/tsunade-20260914/base_basic_pbr.glb',import.meta.url);
...
if(triangles.length/3!==36392)throw new Error('Unexpected Tsunade source topology');
...
const root=new T.Group();root.name='Tsunade_Imported_20260914';
root.userData={source:'gangshouglf.zip / base_basic_pbr.glb',sourceSha256:hash(bytes),extraction:'Complete single character; original topology, PBR textures and UVs',motion:'Added office rig; articulated shoulders, elbows and wrists'};
...
for(const [side,s] of [['Left',1],['Right',-1]]){
  joint(`${side}UpperArm`,'Chest',[s*.215,1.055,0]);
  joint(`${side}ForeArm`,`${side}UpperArm`,[s*.29,.84,-.01]);
  joint(`${side}Hand`,`${side}ForeArm`,[s*.34,.66,.01]);
}
...
for(const [vertex,index] of ids.entries()){
  ...
  else if(y>=.64||(y>=.50&&Math.abs(x)>.26)){const h=smooth(.50,1.02,y);weights={Hips:1-h,Chest:h};}
  else {
  ...
  const arm=hairIslands.has(component(index))?0:armIslands.has(component(index))?1:smooth(.235,.30,Math.abs(x))*smooth(.59,.64,y)*(1-smooth(.92,1.08,y))*smooth(-.145,-.09,z);
  if(arm>0){
...
for(const side of ['Left','Right']){
  ...
  for(let i=0;i<position.count;i++)for(let c=0;c<4;c++)if(indices.getComponent(i,c)===jointIndex&&weights.getComponent(i,c)>.9)handVertices[side].push(i);
  if(!handVertices[side].length)throw new Error('Missing rigid fingertip contact vertices: '+side);
}
const mesh=new T.SkinnedMesh(meshGeometry,new T.MeshStandardMaterial({name:'Tsunade_Imported_PBR'}));
mesh.name='Tsunade_Imported_Body';root.add(mesh);root.updateMatrixWorld(true);mesh.bind(new T.Skeleton(joints));
...
const meta={scale:1,seatOffsetY:.055,standingOffsetY:.055,seatOffsetZ:-.12,typingPull:.20,labelHeight:2.38,actorY:.02,footrestTop:.23,footrestOffsetZ:.245,footrestDepthScale:1.35,seatedHipY:.625,adapter:'Imported single-character PBR mesh; 18-joint office rig',contact:'Articulated shoulders, elbows and wrists; alternating keyboard taps with planted feet.'};
...
function seated(blend=1,time=0,working=false){
  ...
  for(const [side,s] of [['Left',1],['Right',-1]]){
    ...
    const target=rest.get(foot.name).clone().lerp(new T.Vector3(s*.17,.23-.075+rest.get(foot.name).y,.23),blend);
  ...
  if(working){
    for(const [side,sign] of [['Left',1],['Right',-1]]){
      ...
      const target=new T.Vector3(sign*.15,1.09-.075+lift,.51-meta.seatOffsetZ-meta.typingPull);
      for(let pass=0;pass<3;pass++){
        ...
        root.updateMatrixWorld(true);
        const bottom=Math.min(...handVertices[side].map(i=>mesh.getVertexPosition(i,new T.Vector3()).y));
        target.y+=1.034-.075+lift-bottom;
      }
    }
  }
  root.updateMatrixWorld(true);
...
const motion={clips:animations.filter(c=>officeNames[c.name]).map(c=>officeClipJSON(c,'tsunade',officeNames[c.name])),meta};
