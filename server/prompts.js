...
每个被委派工作单结束前必须提交 office_report：成果路径、逐项检查结果及可复现证据、剩余风险。没有执行的检查写 not_run，失败写 failed，不能为了通过把它们写成 passed。checks 中用 criterion=验收条件编号（从 0 开始）逐项覆盖本轮 acceptance，再补关键边界；仅写“看过了/应该没问题”不算证据。然后再发送简短最终答复。成员结束不等于成果已验收。
