...
import {KimiBridge} from './kimi.js';
...
const cliPort=(()=>{const i=process.argv.indexOf('--port');const value=i>=0?Number(process.argv[i+1]):0;return Number.isFinite(value)&&value>0?value:0;})();
const port=Number(process.env.OFFICE_PORT||cliPort||4317),host='127.0.0.1';
const token=randomBytes(32).toString('hex');
const dataDir=process.env.OFFICE_DATA_DIR||path.join(root,'.office-data');
const bridge=new OfficeRuntimes({codex:new CodexBridge({cwd:root}),zcode:new ZcodeBridge({cwd:root}),dsh:new DshBridge({cwd:root}),kimi:new KimiBridge({cwd:root})});
