...
7. **中断了也能继续**：服务重启、运行环境掉线或你手动停止留下的现场，页面顶部会出现「▶ 继续推进」。点它会重新排队被中断的工作单、恢复待命成员，并把明确失败/停止的工作单交给规划者判断重试（`office_retry`，最多两次）或重新规划；超时中断这类可能有外部副作用的操作仍要你先确认，历史证据与已完成成果都保留。
