...
7. **中断了也能继续**：服务重启、运行环境掉线或你手动停止留下的现场，页面顶部会出现「▶ 继续推进」。点它会**先重建掉线的运行环境**（只重启掉线的那个，健康的进程不动），再重新排队工作单、复位成员，并报告哪个环境仍未就绪；某一位成员单独失败时，成员卡片上还有「↻ 重试」只恢复它。左侧「重建本地连接」按钮同样会重建全部（或指定的）运行环境。点它会重新排队被中断的工作单、恢复待命成员，并把明确失败/停止的工作单交给规划者判断重试（`office_retry`，最多两次）或重新规划；超时中断这类可能有外部副作用的操作仍要你先确认，历史证据与已完成成果都保留。
