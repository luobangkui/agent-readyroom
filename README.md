...
7. 每条会话和每个项目右侧都有一个「⋯」菜单：会话可以**重命名**、**归档 / 恢复**；项目可以**重命名**、**从列表移除 / 放回**。左侧顶部在「当前 / 已归档」之外还有「已移除」一栏，专门用来把移除过的项目放回来。
8. **重命名和移除都不动磁盘**：重命名只改列表里的标签（`projects.json` / `missions.json`），目录、对话、消息、工作单、产物全都原样保留，Codex 线程名也不会被改写；「从列表移除」只给项目打一个 `hiddenAt` 时间戳，它在界面上连同底下的会话一起消失，但没有任何文件或记录被删除。手动起的名字会一直生效，不会被第一条消息自动生成的名字覆盖。
9. **中断了也能继续**：服务重启、运行环境掉线或你手动停止留下的现场，页面顶部会出现「▶ 继续推进」。点它会**先重建掉线的运行环境**（只重启掉线的那个，健康的进程不动），再重新排队工作单、复位成员，并报告哪个环境仍未就绪；某一位成员单独失败时，成员卡片上还有「↻ 重试」只恢复它。左侧「重建本地连接」按钮同样会重建全部（或指定的）运行环境。点它会重新排队被中断的工作单、恢复待命成员，并把明确失败/停止的工作单交给规划者判断重试（`office_retry`，最多两次）或重新规划；超时中断这类可能有外部副作用的操作仍要你先确认，历史证据与已完成成果都保留。
...
- `office_report` 的 checks 用 `criterion` 对应 acceptance 的零起始编号；没执行写 `not_run`，失败写 `failed`，不能为通过而写 `passed`。不带 `criterion` 的补充检查不计入验收覆盖；全部通过但有验收条件未被编号覆盖的 `ready` 报告会被直接拒绝并指出缺少的编号，避免下游无限等待证据。
- 写入成果需要另一位成员的只读 `reviewOf` 复核；复核后文件再被修改，复核标记过期。修复用 `replaces` 保留旧记录。
- 缺证据最多自动补全两轮，仍不满足就标为待处理，不会把未验证说成已交付；任务图因阻塞暂停推进时，这两轮补全同样生效，不会被阻塞检查吞掉。
...
- 历史目标不会补造依赖或证据；服务重启不会自动重放中断的模型任务，但页面提供「继续推进」一键恢复：被中断的工作单重新排队，失败/停止的工作单交给规划者判断，需要确认副作用的操作不会被静默重放。
