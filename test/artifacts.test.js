...
test('artifact collection shows changed text files, excludes unchanged, hidden and linked content',async t=>{
 ...
 const baseline=await scanWorkspace(cwd);await writeFile(path.join(cwd,'result.md'),'actual output');await writeFile(path.join(cwd,'credentials.json'),'sensitive');await symlink(path.join(cwd,'old.md'),path.join(cwd,'linked.md'));
