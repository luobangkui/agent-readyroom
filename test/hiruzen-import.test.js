...
const item=officeAvatar('hiruzen-custom');
...
test('Hiruzen is a single character from the supplied GLB with all three original PBR textures',async t=>{
  if(!sourceFixture('../assets/imports/hiruzen-sandaimu2-20260914/base_basic_pbr.glb'))return t.skip('公开版不含第三方原始素材 assets/imports/，本用例只在本地素材齐全时运行');
  const sourceBytes=await readFile(new URL('../assets/imports/hiruzen-sandaimu2-20260914/base_basic_pbr.glb',import.meta.url));
  const bytes=await readFile(path(item.url)),source=unpack(sourceBytes),output=unpack(bytes);
  const manifest=JSON.parse(await readFile(new URL('../public/assets/characters/custom/hiruzen-custom-manifest.json',import.meta.url),'utf8'));
  ...
  assert.equal(manifest.sourceArchive,'sandaimu2.zip');assert.equal(manifest.sourceSha256,hash(sourceBytes));assert.equal(item.modelRevision,hash(bytes).slice(0,12));
  assert.equal(output.doc.meshes.length,1);assert.equal(output.doc.meshes[0].primitives.length,1);
  const primitive=output.doc.meshes[0].primitives[0];
  assert.equal(output.doc.accessors[primitive.indices].count/3,1000000,'the complete supplied character must be preserved');
  assert.equal(output.doc.accessors[primitive.attributes.POSITION].count,657157);
  ...
  assert.deepEqual(accessorBytes(output,primitive.indices),accessorBytes(source,sourcePrimitive.indices),'original topology changed');
  ...
  const gltf=await prepared,mesh=gltf.scene.getObjectByName('Hiruzen_Imported_Body');
...
test('all imported Hiruzen clips animate finite vertices, and loops meet at their endpoints',async()=>{
  const gltf=await prepared,model=createRiggedModel(gltf),mesh=model.root.getObjectByName('Hiruzen_Imported_Body');
...
test('seated soles rest on the extended footrest throughout sitting and working loops',async t=>{
  ...
  actor.hasSeat=actor.wantsSeat=true;actor.setAvatar('hiruzen-custom');await actor.modelReady;
  for(const mode of ['idle','working']){
    actor.mode=mode;for(let i=0;i<120;i++)actor.updateAssetPose(1/60);
    assert.equal(actor.assetMotion.state,mode==='working'?'typing':'sitting');assert.ok(Math.abs(actor.deskPullIn-(mode==='working'?.30:0))<1e-6);
    const mesh=actor.modelRoot.getObjectByName('Hiruzen_Imported_Body'),p=mesh.geometry.attributes.position,meta=actor.modelMeta;
  ...
  actor.setTheme(getTheme('konoha'));await actor.modelReady;assert.equal(actor.modelId,'hiruzen-custom');assert.equal(actor.assetMotion.state,'typing');
});

test('two imported Hiruzen actors have separate rigs and finish standing before walking',async()=>{
...
test('Hiruzen alternates visible hand taps at the keyboard while seated, including idle desk work',async t=>{
  const actor=createCharacter(new T.Group(),'boss',0,0,primitives,getTheme('styloo'),{loader,motionLoader});t.after(()=>actor.disposeAppearance());
  actor.hasSeat=actor.wantsSeat=true;actor.mode='idle';actor.deskTyping=true;actor.setAvatar('hiruzen-custom');await actor.modelReady;
  assert.equal(actor.assetMotion.state,'typing');
  const mesh=actor.modelRoot.getObjectByName('Hiruzen_Imported_Body'),skin=mesh.geometry.attributes.skinIndex,weights=mesh.geometry.attributes.skinWeight;
...
test('replaced Hiruzen models, textures and old source assets are removed',async()=>{
  for(const name of ['assets/imports/hiruzen-20260914','assets/imports/hiruzen-sandaimu-20260914','evidence/hiruzen-import/previous','assets/blender/hiruzen_chibi.blend','assets/blender/rodin-single/hiruzen_rodin_single.glb','assets/textures/hiruzen-face-albedo-v2.png','scripts/build-hiruzen-custom.mjs']){
