...
const item=officeAvatar('tsunade-custom');
...
test('Tsunade is a single character from the supplied GLB with all three original PBR textures',async t=>{
  if(!sourceFixture('../assets/imports/tsunade-20260914/base_basic_pbr.glb'))return t.skip('公开版不含第三方原始素材 assets/imports/，本用例只在本地素材齐全时运行');
  const sourceBytes=await readFile(new URL('../assets/imports/tsunade-20260914/base_basic_pbr.glb',import.meta.url));
  const bytes=await readFile(path(item.url)),source=unpack(sourceBytes),output=unpack(bytes);
  const manifest=JSON.parse(await readFile(new URL('../public/assets/characters/custom/tsunade-custom-manifest.json',import.meta.url),'utf8'));
  ...
  assert.equal(manifest.sourceSha256,hash(sourceBytes));assert.equal(item.modelRevision,hash(bytes).slice(0,12));
  ...
  assert.equal(output.doc.accessors[primitive.indices].count/3,36392,'preserve every source triangle');
  assert.equal(output.doc.accessors[primitive.attributes.POSITION].count,20860);
  ...
  const gltf=await prepared,mesh=gltf.scene.getObjectByName('Tsunade_Imported_Body');
  ...
  assert.ok(size.x<1.2&&size.y>2.2&&size.y<2.3);assert.ok(Math.abs(box.getCenter(new T.Vector3()).x)<.001);
...
test('all imported Tsunade clips animate finite vertices, and loops meet at their endpoints',async()=>{
  const gltf=await prepared,model=createRiggedModel(gltf),mesh=model.root.getObjectByName('Tsunade_Imported_Body');
  ...
  for(const clip of gltf.animations){
    ...
    for(let frame=0;frame<=30;frame++){
      ...
      for(let v=0;v<mesh.geometry.attributes.position.count;v+=47){const point=mesh.getVertexPosition(v,new T.Vector3());assert.ok(point.toArray().every(Number.isFinite));assert.ok(point.length()<4,'mesh explodes during a transition');}
    }
...
test('seated soles rest on the extended footrest throughout sitting and working loops',async t=>{
  ...
  actor.hasSeat=actor.wantsSeat=true;actor.setAvatar('tsunade-custom');await actor.modelReady;
  for(const mode of ['idle','working']){
    actor.mode=mode;for(let i=0;i<120;i++)actor.updateAssetPose(1/60);
    assert.equal(actor.assetMotion.state,mode==='working'?'typing':'sitting');assert.ok(Math.abs(actor.deskPullIn-(mode==='working'?.20:0))<1e-6);
    const mesh=actor.modelRoot.getObjectByName('Tsunade_Imported_Body'),p=mesh.geometry.attributes.position,meta=actor.modelMeta;
    const feet=[-1,1].map(sign=>Array.from({length:p.count},(_,i)=>i).filter(i=>p.getY(i)<.1062&&Math.sign(p.getX(i))===sign));
  ...
  actor.setTheme(getTheme('konoha'));await actor.modelReady;assert.equal(actor.modelId,'tsunade-custom');assert.equal(actor.assetMotion.state,'typing');
});

test('two imported Tsunade actors have separate rigs and finish standing before walking',async()=>{
...
test('Tsunade alternates visible hand taps at the keyboard while seated, including idle desk work',async t=>{
  const actor=createCharacter(new T.Group(),'boss',0,0,primitives,getTheme('styloo'),{loader,motionLoader});t.after(()=>actor.disposeAppearance());
  actor.hasSeat=actor.wantsSeat=true;actor.mode='idle';actor.deskTyping=true;actor.setAvatar('tsunade-custom');await actor.modelReady;
  assert.equal(actor.assetMotion.state,'typing');
  const mesh=actor.modelRoot.getObjectByName('Tsunade_Imported_Body'),skin=mesh.geometry.attributes.skinIndex,weights=mesh.geometry.attributes.skinWeight;
  const hands=['Left','Right'].map(side=>{
    ...
    return Array.from({length:skin.count},(_,i)=>i).filter(i=>[0,1,2,3].some(c=>skin.getComponent(i,c)===bone&&weights.getComponent(i,c)>.9));
  });
  const samples=[[],[]],feet=[];
